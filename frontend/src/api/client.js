...
export async function apiGet(path, { auth = false } = {}) {
  ...
  return request(path, { method: 'GET', headers }, auth)
}
...
export async function describeBrand(description) {
  return apiPost(`${API_V1}/brands/describe`, { description }, { auth: true })
}
...
export async function analyzeBrand(brandId) {
  return apiPost(`${API_V1}/responses/analyze/${brandId}`, {}, { auth: true })
}
...
export async function askBrand(brandId, question) {
  return apiPost(`${API_V1}/chat/${brandId}`, { question }, { auth: true })
}
