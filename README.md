...
| Token | Valeur | Usage |
|-------|--------|-------|
| `--text` | `#57516a` | Texte du corps |
| `--text-h` | `#16121f` | Titres, texte fort |
| `--bg` | `#ffffff` | Fond de page |
| `--border` | `#ece9f3` | Bordures |
| `--accent` | `#6d4aff` | Violet primaire (CTA, liens) |
| `--accent-2` | `#ff4d8d` | Rose secondaire (gradient) |
| `--grad` | `linear-gradient(135deg, accent, accent-2)` | Boutons principaux, avatars, barres de progression |
...
|---------|--------|--------|
| Corps | `system-ui, 'Segoe UI', Roboto` | 18px (16px sur <=1024px) |
| H1 | Même police | 54px, letter-spacing -0.03em |
| H2 | Même police | 26px |
| H3 | Même police | 18px |
...
| Token | Valeur |
|-------|--------|
| Echelle d'espacement | `--space-1` (8px) → `--space-5` (64px) |
| Rayon de bordure | `16px` (defaut), `24px` (grand), `10px` (petit) |
| Ombres | 3 niveaux — `--shadow`, `--shadow-sm`, `--shadow-lg` |

### Composants UI

| Composant | Description |
|-----------|-------------|
| **Boutons** | `.btn-primary` (gradient, pill), `.btn-ghost` (transparent + bordure) |
| **Cartes** | Bordure arrondie, effet hover lift (+4px), ombre |
| **Eyebrow** | Petit tag uppercase en mono, couleur accent |
| **Champs de formulaire** | Flex column, focus ring violette (border + glow 4px) |
| **Navigation** | Sticky, backdrop-filter blur (glass morphism) |
| **Stepper** | Points numerotes avec barres de connexion, etat actif = accent |
| **Chat** | Bulles agent/user, indicateur de frappe anime, suggestion chips |
| **Score de sante** | Anneau SVG anime, code couleur (vert/orange/rouge) |

### Points d'arret responsives

| Largeur | Adaptation |
|---------|------------|
| 1024px | Nav hamburger, grille features → 1 colonne |
| 900px | Panneau aside login masque |
| 720px | Stepper wrappé, labels masques |
| 640px | Grille rapport → 1 colonne |
| 600px | Padding reduit, boutons empiles |
| 520px | Texte hero reduit |
